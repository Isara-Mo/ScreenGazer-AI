...
自动翻译额外保留最近 **128 组完整成功译文**的内存缓存，并合并正在处理的同句请求。即使 OCR 稳定结果出现 A→B→A，缓存中的 A 也能直接恢复，不再发送模型请求；回复选项的内容和顺序一起参与判断，OCR/VL 模式分别缓存。匹配仅规范化空白、全角及引号，不忽略大小写、数字或否定词，不使用模糊相似度。两种模式都以监视识别的文字为依据，因此无法弥补 OCR 漏字；新的识别结果仍需翻译。缓存不保存错误、缺译或警告结果；超过容量会淘汰最久未用的记录，停止监视、修改配置或点击“立即翻译”会清空缓存。手动翻译始终重新请求，方便重试或纠正不满意的译文。

每次翻译完成后，主窗口保留简洁的耗时汇总：**整体 → 确认文字（截图/裁切、累计 OCR 及次数、等待/判断）→ 排队 → 请求 OCR 或复用 → 模型 → 可选术语校正 → 其他处理/显示**；如旧结果曾暂存，还会列出等待对白恢复的耗时。命中缓存时明确显示“本次模型请求 0 次”；合并进行中的请求时显示剩余等待及原请求耗时，原请求跨越本轮检测的部分不重复计入。“文字保持一致”与确认过程重叠，明确注明不再相加。整体从第一次采到这句文字的截图开始，手动翻译则从开始准备采集算起，到程序将结果写入文本框结束；监视中手动触发时，不包含采集前等待上一轮 OCR 结束的时间。整体也不包含无法测量的“游戏实际换句到首次采样”间隔，不代表显示器实际呈现的时延。“请求合计”仍单独保留，不包含此前监视和排队。

以上为应用层去重，不等于所有情况只产生一次 HTTP 请求：OpenAI 兼容客户端所用 SDK 默认会对部分网络错误、限流及服务端错误最多重试两次；DashScope 的接口/图像格式兼容回退最多追加一次；VL 的术语校正最多追加一次。成功的普通请求不会因为本地 OCR 周期性复查而重发。
